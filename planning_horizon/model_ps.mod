...
var r_sm_i {t in 0..T, s in 1..6, m in 1..M} = bev_holt[t] * ssb_scaled_s[t, s, m] / (ssb_scaled_total[t]+eps);  # with hyperallometric scaling
...
var mean_weight_before_harvest {t in 0..T} = sum{s in 1..6} (w_mean[s] * (sum{m in 1..M}survivors_natural[t, s, m]) / (eps+sum{j in 1..6, m in 1..M}survivors_natural[t, j, m]));

# --------- 4. fishing ---------
var h_s {t in 0..T, s in 1..6}>=0;
...
var h_sm {t in 0..T, s in 1..6, m in 1..M} = h_s[t, s] * (a_N[t,s,m] + eps) / (sum{j in 1..M} (a_N[t,s,j] + eps));
...
# --------- optimise ---------
maximize discounted_smoothed_surplus: sum{t in 0..T} (delta^t * surplus[t])^(smoother);  
maximize nothing: 0;
